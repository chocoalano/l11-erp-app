        ...
        @foreach($record->team as $member)
            <div class="flex items-center gap-x-2.5">
                <div class="flex gap-1.5">
                    <img src="{{ asset('storage/'.$member->image) }}" style="height: 2.5rem; width: 2.5rem;" class="max-w-none object-cover object-center rounded-full ring-white dark:ring-gray-900" alt="{{ $member->name }}">
                </div>
            </div>
        @endforeach
    </div>
